...
bool timeout(clock_t timer_sample, bool timer_enable, bool dyn_timer_enable, double *timer, int *trial_counter){
    ...
    if((timer_enable) && (timeout)){ 
      ...
      if(dyn_timer_enable){
        if(2*(*timer) > 30000) {
          *timer = 2000; //timer troppo elevato provo a "sbloccare" il canale (forzandolo) e limitare l'attesa
        }
        else {
 ...
 //Ricevo dati
  while(1){
    ...
    if(!simulate_loss(loss_rate)){
      ...
      printf("ACK %d inviato di tipo %d\n", ntohl(ack.seq_no), ack.type);
   }
    else
